...
function ppom_hooks_load_input_scripts( $product, $ppom_id = null ) {
    ...
    wp_enqueue_script( 'ppom-price', PPOM_URL . "/js/price/{$ppom_price_js}", array(
            'jquery',
            'ppom-inputs'
    ), PPOM_VERSION, true );
    ...
    if ( ! empty( $ppom_conditional_fields ) || apply_filters( 'ppom_enqueue_conditions_js', false ) ) {
        ...
        wp_enqueue_script( "ppom-{$ppom_conditions_script}", PPOM_URL . "/js/{$ppom_conditions_script}.js", array(
                'jquery',
                'ppom-inputs'
        ), PPOM_VERSION, true );
...
function ppom_hooks_register_wpml( $meta_data, $ppom_id ) {
    ...
    foreach ( $meta_data as $index => $data ) {
        ...
        if ( isset( $data['images'] ) && is_array( $data['images'] ) ) {
            ...
            $data['images'] = array_map( function ( $option ) {
