...
test.describe( 'Group Fields Edit', () => {
	...
	test( 'change select option order on saving', async ( { page, admin } ) => {
		...
		await page.getByRole( 'textbox' ).fill( 'Change Select Option order' );
		...
		await page.evaluate( ( modelId ) => {
			const tbody = document.querySelector(
				`#ppom_field_model_${ modelId } ul.ppom-options-sortable`
			);

			const rows = Array.from(
				tbody.querySelectorAll( 'li.ui-sortable-handle' )
			);
			if ( rows.length > 1 ) {
				const lastRow = rows[ rows.length - 1 ];
				tbody.insertBefore( lastRow, rows[ 0 ] );
