...
test.describe( 'Attach Modal', () => {
	...
	test( 'attach to multiple categories', async ( { page, admin } ) => {
		...
		await page.evaluate( () => {
			document
				.querySelector( '#attach-to-categories > div.postbox' )
	 ...
	 */
	test( 'attach modal visibility on Group Edit page', async ( {
		page,
		admin,
	} ) => {
		await admin.visitAdminPage( 'admin.php?page=ppom' );
		...
		await page
			.getByRole( 'textbox' )
			.fill( 'Test Attach Modal visibility' );

		await expect(
			page.locator( '[data-formmodal-id="ppom-product-modal"]' )
		).toBeHidden();

		await addNewField( page );
		await pickFieldTypeInModal( page, 'text' );
		await fillFieldNameAndId( page, 1, `Test`, `test` );
