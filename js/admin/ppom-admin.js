...
const FIELD_COMPATIBLE_WITH_SELECT_OPTIONS = [
	'select',
	'radio',
	'switcher',
	'image',
...
const FIELDS_COMPATIBLE_WITH_NUMBERS = [
...
const COMPARISON_VALUE_CAN_USE_SELECT = [
	'is',
	'not',
	'greater than',
	'less than',
];
const HIDE_COMPARISON_INPUT_FIELD = [
	'any',
	'empty',
	'odd-number',
	'even-number',
...
jQuery( function ( $ ) {
	...
	$( document ).on(
		'click',
		'[data-modal-id]:not(.ppom-is-pro-field)',
		function ( e ) {
			e.preventDefault();
			$( 'body' ).append( append_overly_model );
			const modalBox = $( this ).attr( 'data-modal-id' );
	...
	function ppom_close_popup() {
		...
		$( '.ppom-js-modal-close, .ppom-modal-overlay' ).click( function ( e ) {
			...
			$( '.ppom-modal-box, .ppom-modal-overlay' ).fadeOut(
				'fast',
				function () {
	 ...
	 */
	let field_no = $( '#field_index' ).val();

	/**
	  4- Show And Hide Visibility Role Field
	 */
	$( '.ppom-slider' )
		.find( '[data-meta-id="visibility_role"]' )
		.removeClass( 'ppom_handle_fields_tab' )
		.hide();
	$( '.ppom_save_fields_model .ppom-slider' ).each( function ( i, div ) {
		const visibility_value = $( div )
			.find( '[data-meta-id="visibility"] select' )
			.val();
	...
	$( document ).on(
		'change',
		'[data-meta-id="visibility"] select',
		function ( e ) {
			e.preventDefault();

			const div = $( this ).closest( '.ppom-slider' );
			const visibility_value = $( this ).val();
				...
				window?.ppomPopup?.open( {
					...
					onConfirmation: () => {
						inputsToRemove.forEach( ( meta_field ) => {
							...
							meta_field
								.closest( `.row_no_${ field_id }` )
								?.remove();
							document
								.querySelector(
									`#ppom_field_model_${ field_id }`
								)
								?.remove();
						} );
					},
	...
	$( document ).on(
		'click',
		'.ppom-edit-field:not(.ppom-is-pro-field)',
		function ( event ) {
			event.preventDefault();

			const the_id = $( this ).attr( 'id' );
			$( '#ppom_field_model_' + the_id )
				.find( '.ppom-close-checker' )
				.removeClass( 'ppom-close-fields' );
			lockedDataName = true;
		}
	);
	...
	$( document ).on( 'click', '.ppom-add-field', function ( event ) {
		...
		const $this = $( this );
		const ui = ppom_required_data_name( $this, 'new' );
		if ( ui == false ) {
			return;
		}

		const copy_model_id = $( this ).attr( 'data-copy-model-id' );
		let id = $( this ).attr( 'data-field-index' );
		id = Number( id );
		// console.log(id);

		const field_title = $( '#ppom_field_model_' + id + '' )
			.find( '.ppom-modal-body .ppom-fields-actions' )
			.attr( 'data-table-id' );
		const data_name = $( '#ppom_field_model_' + id + '' )
			.find( '[data-meta-id="data_name"] input' )
			.val();
		const title = $( '#ppom_field_model_' + id + '' )
			.find( '[data-meta-id="title"] input' )
			.val();
		let placeholder = $( '#ppom_field_model_' + id + '' )
			.find( '[data-meta-id="placeholder"] input' )
			.val();
		const required = $( '#ppom_field_model_' + id + '' )
			.find( '[data-meta-id="required"] input' )
			.prop( 'checked' );
		...
		let html =
			'<tr class="row_no_' +
			id +
			' ui-sortable-handle" id="ppom_sort_id_' +
			id +
			'">';
		html +=
		...
		html +=
			...
			'" tabindex="0">';
		html +=
			'<label class="onoffswitch-label" for="ppom-onoffswitch-' +
			id +
			'">';
		...
		if ( copy_model_id != '' && copy_model_id != undefined ) {
			$( html )
				.find( '.ppom_field_table tbody' )
				.end()
				.insertAfter( '#ppom_sort_id_' + copy_model_id + '' );
		} else {
		...
		$( this )
			.removeClass( 'ppom-add-field' )
			.addClass( 'ppom-update-field' );
	...
	$( document ).on( 'click', '.ppom-update-field', function ( event ) {
		...
		const $this = $( this );
		const ui = ppom_required_data_name( $this, 'update' );
		...
		const title = $( '#ppom_field_model_' + id + '' )
			.find( '[data-meta-id="title"] input' )
			.val();
		const placeholder = $( '#ppom_field_model_' + id + '' )
			.find( '[data-meta-id="placeholder"] input' )
			.val();
		const required = $( '#ppom_field_model_' + id + '' )
			.find( '[data-meta-id="required"] input' )
		...
		const row = $( '.ppom_field_table tbody' ).find( '.row_no_' + id );
	...
	$( document ).on( 'click', '.ppom_select_field', function ( event ) {
		...
		$( '#ppom_fields_model_id' )
			.find( '.ppom-js-modal-close' )
			.trigger( 'click' );

		const field_type = $( this ).data( 'field-type' );
		const clone_new_field = $(
		...
		clone_new_field
			.find( '.ppom-fields-actions [data-meta-id="visibility_role"]' )
			.hide();

		const field_model_id = 'ppom_field_model_' + field_no + '';

		clone_new_field
			.find( '.ppom_save_fields_model' )
			.end()
			.appendTo( '.ppom_save_fields_model' )
			.attr( 'id', field_model_id )
			.find( '.ppom-tabs-header label.ppom-tabs-label' )
			.each( function ( index, item ) {
				const tabId = $( item ).attr( 'id' );
				const hasTabOptions =
					$( '#' + field_model_id, document )?.find(
						'.ppom_handle_' + tabId
					)?.length > 0;
				if ( ! hasTabOptions ) {
					$( item ).hide();
		...
		clone_new_field
			.find( '.ppom-field-checker' )
			.attr( 'data-field-index', field_no );
		clone_new_field
			.find( '.ppom-field-checker' )
			.addClass( 'ppom-add-fields-js-action' );
		...
		const field_index = field_no;

		// handle multiple options
		const ppom_option_type = '';
		const option_selector = clone_new_field.find( '.ppom-option-keys' );
		const option_controller = clone_new_field.find( '.ppom-fields-option' );
		const add_cond_selector = clone_new_field.find(
			'.ppom-conditional-keys'
		);

		// for address addon
		const address_selector = clone_new_field.find( '.ppom-checkout-field' );
		const address_table_id = clone_new_field.find( '.ppom_address_table' );
		...
		const wpcolor_selector = clone_new_field.find(
			'.ppom-color-picker-cloner'
		);
		ppom_wp_color_handler( wpcolor_selector, field_index, option_index );

		ppom_create_option_index(
			option_selector,
			field_index,
			option_index,
			ppom_option_type
		...
		$( document ).trigger( 'ppom_new_field_created', [
			clone_new_field,
			field_no,
			field_type,
		] );
	...
	$( '.ppom-main-field-wrapper' ).on(
		'click',
		'.ppom_copy_field:not(.ppom-is-pro-field)',
		function ( e ) {
			e.preventDefault();

			const model_id_no = $( this ).attr( 'id' );

			const field_type = $( this ).data( 'field-type' );
			// console.log(model_id_no);

			const clone_new_field = $(
				'.ppom_save_fields_model #ppom_field_model_' + model_id_no + ''
			).clone( true );
			const dataTitleField = clone_new_field.find(
				'[data-metatype="title"]'
			);
			const dataNameField = clone_new_field.find(
				'[data-metatype="data_name"]'
			);
			let dataNameOldVal = dataNameField.val();
			const duplicateFields = jQuery( document ).find(
				'.ppom_field_table .ppom_meta_field_title:contains(' +
					dataTitleField.val() +
					')'
			);
			let dataNameNewVal = '';
			...
			if ( duplicateFields && duplicateFields.length >= 1 ) {
				...
				dataNameNewVal =
					dataNameOldVal + '_copy_' + duplicateFields.length++;
			} else {
			...
			clone_new_field
				.find( '.ppom_save_fields_model' )
				.end()
				.insertAfter( '#ppom_field_model_' + model_id_no + '' )
				.attr( 'id', 'ppom_field_model_' + field_no + '' );
			clone_new_field
				.find( '.ppom-add-fields-js-action' )
				.attr( 'data-field-index', field_no );
			clone_new_field
				.find( '.ppom-close-fields' )
				.attr( 'data-field-index', field_no );
			clone_new_field
				.find( '.ppom-js-modal-close' )
				.addClass( 'ppom-close-fields' );
			clone_new_field
				.find( '.ppom-add-fields-js-action' )
				.removeClass( 'ppom-update-field' );
			clone_new_field
				.find( '.ppom-add-fields-js-action' )
				.attr( 'data-copy-model-id', model_id_no );
			clone_new_field
				.find( '.ppom-add-fields-js-action' )
				.addClass( 'ppom-add-field' );
			clone_new_field
				.find( '.ppom-add-fields-js-action' )
				.addClass( 'ppom-insertafter-field' );
			clone_new_field
				.find( '.ppom-add-fields-js-action' )
				.html( 'Add Field' );
			...
			// handle multiple options
			const ppom_option_type = 'ppom_copy_option';
			const option_selector = clone_new_field.find( '.ppom-option-keys' );
			const add_cond_selector = clone_new_field.find(
				'.ppom-conditional-keys'
			);
			const eventcalendar_selector = clone_new_field.find(
				'.ppom-eventcalendar-field'
			);
			const image_option_selector = clone_new_field.find(
				'[data-table-id="image"] .data-options, [data-table-id="imageselect"] .data-options'
			);

			// reset option to one
			// clone_new_field.find('[data-table-id="image"] .data-options').remove();
			clone_new_field
				.find( '[data-table-id="audio"] .pre-upload-box li' )
				.remove();
			...
			$( clone_new_field )
				.find( 'select[data-metatype="elements"]' )
				.each( function ( i, condition_element ) {
					const existing_value1 =
						$( condition_element ).attr( 'data-existingvalue' );
					if ( $.trim( existing_value1 ) !== '' ) {
						jQuery( condition_element ).val( existing_value1 );
					}
				} );
			$( clone_new_field )
				.find( 'select[data-metatype="element_values"]' )
				.each( function ( i, condition_element ) {
					const div = $( this ).closest( '.webcontact-rules' );
					const existing_value1 =
						$( condition_element ).attr( 'data-existingvalue' );
			...
			const option_controller = clone_new_field.find(
				'.ppom-fields-option'
			);
			...
			image_option_selector
				.find( 'input' )
				.each( function ( img_index, img_meta ) {
					const opt_in = $( img_meta ).attr( 'data-opt-index' );
					const field_name =
						'ppom[' +
						field_index +
						'][images][' +
	...
	popupForm?.addEventListener( 'submit', ( e ) => {
		...
		const formContent = new FormData(
			document.querySelector( '#ppom-product-form' )
		);
		...
		fetch( ajaxurl, {
			...
			.then( ( resp ) => {
				...
				if ( openModalBtn && openModalBtn.dataset?.reload ) {
					...
					document
						.querySelector( '.ppom-js-modal-close' )
	...
	$( document ).on( 'click', '.ppom-tabs-label', function () {
		...
		const fields_wrap = selectedTab.parent();
		selectedTab.find( '.ppom-tabs-label' ).removeClass( 'ppom-active-tab' );
		$( this ).addClass( 'ppom-active-tab' );
		const content_box = fields_wrap.find( '.ppom-control-all-fields-tabs' );
		content_box.hide();
	...
	$( document ).on( 'click', '.ppom-pre-upload-image-btn', function ( e ) {
		...
		let meta_type = $( this ).attr( 'data-metatype' );
		$uploaded_image_container = $( this ).closest( 'div' );
		const image_append = $uploaded_image_container.find( 'ul' );
		let option_index = parseInt(
			$uploaded_image_container.find( '#ppom-meta-opt-index' ).val()
		);
		const main_wrapper = $( this ).closest( '.ppom-slider' );
		const field_index = main_wrapper
			.find( '.ppom-fields-actions' )
			.attr( 'data-field-no' );
		let price_placeholder = ppom_vars.i18n.pricePlaceholder;

		let wp_media_type = 'image';
		if ( meta_type == 'audio' ) {
		...
		var button = $( this ),
			custom_uploader = wp
				...
				.on( 'select', function () {
					const attachments = custom_uploader
						.state()
						.get( 'selection' )
						.toJSON();
					...
					attachments.map( ( meta, index ) => {
						...
						let price_metatype = 'price';
						const stock_metatype = 'stock';
						const stock_placeholder = ppom_vars.i18n.stock;
						let url_field =
							'<input placeholder="url" type="text" name="ppom[' +
							field_index +
							'][' +
							meta_type +
							'][' +
							option_index +
							'][url]" class="form-control" data-opt-index="' +
							option_index +
							'" data-metatype="url" value="">';
						...
						if ( meta_type == 'imageselect' ) {
							...
							url_field =
								'<input placeholder="Description" type="text" name="ppom[' +
								field_index +
								'][' +
								meta_type +
								'][' +
								option_index +
								'][description]" class="form-control" data-opt-index="' +
								option_index +
								'" data-metatype="description">';
						...
						if ( fileurl ) {
							...
							image_box +=
								...
								'">';
							image_box +=
								'<span class="dashicons dashicons-move" style="margin-bottom: 7px;margin-top: 2px;"></span>';
							image_box +=
								'<span class="ppom-uploader-img-title"></span>';
							image_box += '<div style="display: flex;">';
							image_box +=
								'<div class="ppom-uploader-img-center">';
							image_box += img_icon;
							image_box += '</div>';
							image_box +=
								'<input type="hidden" name="ppom[' +
								field_index +
								'][' +
								meta_type +
								'][' +
							...
							image_box +=
								...
								'" data-metatype="id">';
							image_box +=
								'<input type="text" placeholder="Title" name="ppom[' +
								field_index +
								'][' +
								meta_type +
								'][' +
								option_index +
								'][title]" class="form-control ppom-image-option-title" data-opt-index="' +
								option_index +
								'" data-metatype="title" value="' +
								file_title +
								'">';
							image_box +=
								...
								'][' +
								price_metatype +
								']" class="form-control" data-opt-index="' +
								option_index +
								'" data-metatype="' +
								price_metatype +
								'" value="">';
							...
							image_box += url_field;
							image_box +=
								'<button class="btn btn-danger ppom-pre-upload-delete" style="height: 35px;"><i class="fa fa-times" aria-hidden="true"></i></button>';
							image_box += '</div>';
							image_box += '</li>';
	...
	$( document ).on( 'click', '.ppom-pre-upload-image-btnsd', function ( e ) {
		...
		wp.media.editor.send.attachment = function ( props, attachment ) {
			...
			let img_icon =
				'<img width="60" src="' + fileurl + '" style="width: 34px;">';
	 ...
	 */
	$( document )
		.on( 'click', '.ppom-add-rule', function ( e ) {
			e.preventDefault();

			const div = $( this ).parents( '.form-group' );
			const option_index = parseInt(
				div.find( '.ppom-condition-last-id' ).val()
			);
			div.find( '.ppom-condition-last-id' ).val( option_index + 1 );

			const field_index = div
				.parents( '.ppom-slider' )
				.find( '.ppom-fields-actions' )
				.attr( 'data-field-no' );
			const cloneElement = $( '.webcontact-rules:last', div );
			const condition_clone = cloneElement.clone();

			const append_item = div.find( '.ppom-condition-clone-js' );
			condition_clone.find( append_item ).end().appendTo( append_item );

			ppom_rename_rules_name();

			const field_type = '';
			const add_cond_selector = condition_clone.find(
				'.ppom-conditional-keys'
			);
			ppom_add_condition_set_index(
				add_cond_selector,
			...
			$( '.ppom-add-rule', cloneElement )
				.removeClass( 'ppom-add-rule' )
				.addClass( 'ppom-remove-rule' )
				.removeClass( 'btn-success' )
				.addClass( 'btn-danger' )
				.html( '<i class="fa fa-minus" aria-hidden="true"></i>' );

			// Last row.
			const lastRow = jQuery( '.webcontact-rules:visible:last' );
			if ( lastRow?.find( '.ppom-remove-rule' )?.length === 0 ) {
				const cloneButton = lastRow?.find( '.ppom-add-rule' ).clone();
				cloneButton
					.removeClass( 'ppom-add-rule' )
					.addClass( 'ppom-remove-rule ml-1' )
					.removeClass( 'btn-success' )
					.addClass( 'btn-danger' )
					.html( '<i class="fa fa-minus" aria-hidden="true"></i>' );
		...
		.on( 'click', '.ppom-remove-rule', function ( e ) {
			...
			if (
				removeButton
					...
					?.find( '.webcontact-rules' )?.length === 1
			) {
				removeButton
					.parents( '.ppom-condition-clone-js' )
			...
			if ( lastRow?.find( '.ppom-add-rule' )?.length === 0 ) {
				const cloneButton = lastRow
					?.find( '.ppom-remove-rule' )
					.clone();
				cloneButton
					.removeClass( 'ppom-remove-rule' )
					.addClass( 'ppom-add-rule' )
					.removeClass( 'btn-danger' )
					.addClass( 'btn-success' )
					.html( '<i class="fa fa-plus" aria-hidden="true"></i>' );
				cloneButton.insertBefore(
					lastRow?.find( '.ppom-remove-rule' )
				);
			}
			ppom_rename_rules_name();
			e.preventDefault();
	 ...
	 */
	$( document )
		.on( 'click', '.ppom-add-option', function ( e ) {
			e.preventDefault();

			const main_wrapper = $( this ).closest( '.ppom-slider' );
			const ppom_option_type = 'ppom_new_option';

			const li = $( this ).closest( 'li' );
			const ul = li.closest( 'ul' );
			const clone_item = li.clone();

			clone_item.find( ul ).end().appendTo( ul );

			const option_index = parseInt(
				ul.find( '#ppom-meta-opt-index' ).val()
			);
			...
			const field_index = main_wrapper
				.find( '.ppom-fields-actions' )
				.attr( 'data-field-no' );
			const option_selector = clone_item.find( '.ppom-option-keys' );
			const option_controller = clone_item.find( '.ppom-fields-option' );
		...
		.on( 'click', '.ppom-remove-option', function ( e ) {
			const selector_btn = $( this ).closest( '.ppom-slider' );
			const option_num = selector_btn.find( '.data-options' ).length;
	...
	$( document ).on( 'keyup', '.option-title', function () {
		const closes_id = $( this ).closest( 'li' ).find( '.option-id' );
		let option_id = $( this )
			.val()
			.replace( /[^A-Z0-9]/gi, '_' );
	...
	$( document ).on(
		'keyup',
		'[data-meta-id="title"] input[type="text"]',
		function () {
			/**
			...
			field_id =
				field_id.charAt( 0 ) === '_'
					? `${ START_CHAR_DISALLOW_BEING_HIDDEN_FIELD }${ field_id }`
					: field_id;

			const selector = $this.closest( '.ppom-slider' );

			const wp_field = selector
				.find( '.ppom-fields-actions' )
				.attr( 'data-table-id' );
			if (
				wp_field == 'shipping_fields' ||
				wp_field == 'billing_fields'
			) {
				return;
			}
			...
			selector
				.find(
					'[data-meta-id="data_name"] input[type="text"]:not([readonly])'
				)
				.val( field_id );
		}
	);
	...
	function insertAt( parent, element, index, dir ) {
		const el = parent.children().eq( index );
	...
	function ppom_required_data_name( $this, context ) {
		const selector = $this.closest( '.ppom-slider' );
		const data_name = selector
			.find( '[data-meta-id="data_name"] input[type="text"]' )
			.val();
		const savedDataName = selector
		...
		const allDataName = $( document )
			...
			.map( function () {
				...
				if (
					$this.hasClass( 'ppom-update-field' ) &&
					data_name === metaFieldId
				) {
					return '';
				}
				return metaFieldId;
		...
		if ( data_name == '' ) {
			...
			( 'new' === context ||
				( 'update' === context && savedDataName !== data_name ) ) &&
			$.inArray( data_name, allDataName ) != -1
		) {
	 ...
	 * @param wpcolor_selector
	 * @param field_index
	 * @param option_index
	 */
	function ppom_wp_color_handler(
		wpcolor_selector,
		field_index,
		option_index
	) {
		wpcolor_selector.each( function ( i, meta_field ) {
			const color_picker_input = $( meta_field )
				.find( '.ppom-color-picker-init' )
				.clone();
	 ...
	 * @param field_index
	 * @param option_index
	 * @param ppom_option_type
	 */
	function ppom_create_option_index(
		option_selector,
		field_index,
		option_index,
		ppom_option_type
		...
		option_selector.each( function ( i, meta_field ) {
			if ( ppom_option_type == 'ppom_copy_option' ) {
				const opt_in = $( meta_field ).attr( 'data-opt-index' );
				if ( opt_in !== undefined ) {
					option_index = opt_in;
				}
			}
	...
	function ppom_option_controller(
		option_selector,
		field_index,
		option_index,
		ppom_option_type
		...
		option_selector.each( function ( i, meta_field ) {
			...
			const field_name =
				...
				field_index +
				'][' +
				$( meta_field ).attr( 'data-optiontype' ) +
				'][' +
				option_index +
				'][' +
				$( meta_field ).attr( 'data-metatype' ) +
	 ...
	 * @param add_c_selector
	 * @param opt_field_no
	 * @param field_type
	 * @param opt_no
	 */
	function ppom_add_condition_set_index(
		add_c_selector,
		opt_field_no,
		field_type,
		opt_no
	...
	// address addon
	function ppom_create_address_index(
		address_selector,
		field_index,
		address_table_id
	) {
		address_selector.each( function ( i, meta_field ) {
			const field_id = $( meta_field ).attr( 'data-fieldtype' );
			const core_field_type =
				$( address_table_id ).attr( 'data-addresstype' );
			const field_name =
				'ppom[' +
				field_index +
				'][' +
	...
	function ppom_eventcalendar_set_index( add_c_selector, opt_field_no ) {
		add_c_selector.each( function ( i, meta_field ) {
			const date = $( meta_field ).attr( 'data-date' );
			const field_name =
				'ppom[' +
				opt_field_no +
				'][calendar][' +
	...
	function ppom_rename_rules_name() {
		$( '.webcontact-rules:visible' ).each( function ( index, item ) {
			$( this )
				.attr( 'id', 'rule-box-' + parseInt( index + 1 ) )
				.find( 'label' )
				.text( function ( i, txt ) {
		...
		operatorSelectField
			...
			.forEach( ( optgroup ) => {
				...
				optgroup.querySelectorAll( 'option' ).forEach( ( option ) => {
					const isAvailable =
						option?.value &&
						OPERATORS_FIELD_COMPATIBILITY[ option.value ]
				...
				optgroup.classList.toggle(
					'ppom-hide-element',
					shouldHideGroup
				);
			} );

		operatorSelectField.classList.toggle(
			'ppom-invisible-element',
			shouldHideSelectInput
		);
	...
	function updateTargetComparisonValueSelect(
		...
		const targetSelectOptions = conditionContainer?.querySelector(
			'select[data-metatype="element_values"]'
		);
		...
		document.querySelectorAll( '.ppom-slider' ).forEach( ( sliderItem ) => {
			const targetElementFieldId = sliderItem.querySelector(
				'input[data-metatype="data_name"]'
			)?.value;
			if ( targetElementFieldId !== targetElementNameToPullOptions ) {
				return;
			}

			const operatorsInput = conditionContainer.querySelector(
			...
			sliderItem.querySelectorAll( '.data-options' ).forEach(
				/** @type {HTMLDivElement} */ ( conditionValueContainer ) => {
					const condition_type = conditionValueContainer.getAttribute(
						'data-condition-type'
					);
	...
	function tryToggleConditionInputFields( conditionOperatorInput ) {
		 ...
		 */
		const container =
			conditionOperatorInput?.closest( '.webcontact-rules' );
		if ( ! container ) {
			return;
		}
		 ...
		 */
		const conditionTargetSelectOptionsInput = container.querySelector(
			'select[data-metatype="element_values"]'
		);
		 ...
		 */
		const conditionConstantInput = container.querySelector(
			'[data-metatype="element_constant"]'
		);

		/**
		 * @type {HTMLSelectElement|null}
		 */
		const conditionTargetSelectInput = container.querySelector(
		 ...
		 * @type {HTMLDivElement|null}
		 */
		const betweenInputs = container.querySelector(
		...
		if ( proOperatorOptionsToLock.has( selectedOperator ) ) {
			 ...
			 */
			const targetFieldTypeInput =
				conditionTargetSelectInput.querySelector(
					`option[value="${ conditionTargetSelectInput.value }"]`
				);
			if (
		...
		if (
			shouldHideSelectInput &&
			shouldHideTextInput &&
			shouldHideBetweenInputs &&
			shouldHideUpsell
		) {
			conditionConstantInput.parentNode?.classList.add(
				'ppom-invisible-element'
			); // NOTE: Make the entire container visible to preserve the space.
		} else {
			conditionTargetSelectOptionsInput?.classList.toggle(
				'ppom-hide-element',
				shouldHideSelectInput
			);
			conditionConstantInput.classList.toggle(
				'ppom-hide-element',
				shouldHideTextInput
			);
			betweenInputs?.classList.toggle(
				'ppom-hide-element',
				shouldHideBetweenInputs
			);
			container
				.querySelector( '.ppom-upsell-condition' )
				?.classList.toggle( 'ppom-hide-element', shouldHideUpsell );

			conditionConstantInput.parentNode?.classList.remove(
				'ppom-invisible-element'
			);
		}
	}
	...
	document
		.querySelectorAll( 'select[data-metatype="operators"]' )
		.forEach( ( conditionOperatorInput ) => {
	...
	$( document ).on(
		'change',
		'[data-meta-id="conditions"] select[data-metatype="element_values"]',
		function ( e ) {
			e.preventDefault();

			const element_values = $( this ).val();
	 ...
	 * @return
	 */
	function populate_condition_target(
		selectInput,
		conditionOperator,
		excludeIds = []
	) {
		...
		const newOptions = availableConditionTargets
			.filter(
				( { fieldId, canUse } ) =>
					canUse && ! excludeIds.includes( fieldId )
			)
			.map( ( target ) => {
	...
	function can_use_field_type( fieldType ) {
		...
		for ( const operatorCompatibleFields of Object.values(
			OPERATORS_FIELD_COMPATIBILITY
		) ) {
	...
	function populate_conditional_elements() {
		...
		document.querySelectorAll( '.ppom-slider' ).forEach( ( item ) => {
			const fieldLabel = item.querySelector(
				'input[data-metatype="title"]'
			)?.value;
			const fieldId = item
				.querySelector( 'input[data-metatype="data_name"]' )
				?.value?.trim();
			const fieldType = item.querySelector(
				'input[data-metatype="type"]'
			)?.value;
			...
			availableConditionTargets.push( {
				fieldLabel,
				fieldId,
				fieldType,
				canUse,
	...
	document.addEventListener( 'change', function ( e ) {
		...
		const conditionOperatorSelect = conditionContainer?.querySelector(
			'[data-metatype="operators"]'
		);
		...
		toggleOperatorFieldByTargetType(
			findFieldTypeById( e.target?.value ),
			conditionOperatorSelect
		);
		...
		const constantInput = conditionContainer.querySelector(
			'[data-metatype="element_constant"]'
		);
	...
	function validate_api_wooproduct( form ) {
		jQuery( form )
			.find( '#nm-sending-api' )
			.html( '<img src="' + nm_personalizedproduct_vars.doing + '">' );
	...
	$( document ).on(
		...
		( e, id, tab ) => {
			...
			$(
				`<div class="form-group">${ ppom_vars.i18n.freemiumCFRContent }</div>`
			).insertAfter( tab.find( '.form-group' ) );
		}
	);
	...
	$( 'input[data-metatype="jquery_dp"]' ).change(
		toggleHandler.activateHandler
	);

	$( document ).on(
		'ppom_new_field_created',
		function ( e, clone_new_field ) {
			$( clone_new_field )
				.find( 'input[data-metatype="jquery_dp"]' )
				.change( toggleHandler.activateHandler );
		}
	);
	...
	$( document ).on(
		...
		function () {
			if (
				...
				$( this ).hasClass( 'ppom-add-fields-js-action' )
			) {
				unsaved = true;
				return;
			}
	...
	$( document ).on( 'ppom_fields_tab_changed', function ( e, id, tab ) {
		...
		if (
			! $( 'input[data-metatype="logic"]', tab?.first() )?.is(
				':checked'
			)
		) {
	...
	$( document ).on(
		'change',
		'input[data-metatype="logic"]:visible',
		function () {
			$( this )
	...
	$( document ).on(
		'click',
		'button.ppom-edit-field.ppom-is-pro-field, button.ppom_copy_field.ppom-is-pro-field',
		function () {
	...
	$( document ).ready( function () {
		$( '.ppom-slider' ).each( function ( i, item ) {
			const itemEl = $( item );
			const value = itemEl
				.find( 'input[data-metatype="data_name"]' )
				.val();
			...
			toggleHandler.setDisabledFields(
				itemEl.find( 'input[data-metatype="jquery_dp"]' )
			);
		} );
	} );
...
document.querySelectorAll( '.ppom-modal-shortcuts a' )?.forEach( ( anchor ) => {
	anchor.addEventListener( 'click', function ( e ) {
		...
		if ( targetId === '#all' ) {
			// Show all sections
			document
				.querySelectorAll( '.ppom-fields-section' )
				.forEach( ( section ) => {
 ...
 */
document
	.querySelector( 'input[name="ppom-search-field"]' )
