...
final class Handler {
	...
	public static function get_dir_path( $sub_dir = false ) {

		$ppom_upload_dir = self::files_setup_get_directory( $sub_dir );

		return apply_filters( 'ppom_dir_path', $ppom_upload_dir );
	}

	// Return upload file dir url
	public static function get_dir_url( $thumb = false ) {
	...
	public static function create_thumb_for_meta( $file_name, $product_id, $cropped = false, $size = null ) {
		...
		if ( is_admin() && $post_type == 'shop_order' ) {
			...
			$file_thumb_url = self::is_file_image( $file_dir_path ) ? self::get_dir_url( true ) . $file_name : PPOM_URL . '/images/file.png';
		}
		...
		$file_path_edit = self::get_dir_path() . 'edits/' . $file_name;

		if ( file_exists( $file_path_edit ) ) {

			$file_link         = self::get_dir_url() . 'edits/' . $file_name;
			$edited_thumb_path = self::get_dir_path() . 'edits/thumbs/' . $file_name;
			if ( file_exists( $edited_thumb_path ) ) {
				$file_thumb_url = self::get_dir_url() . 'edits/thumbs/' . $file_name;
			}
		...
		$ppom_html .= '<td style="max-width:300px;text-align:left;">' . esc_attr( self::files_trim_name( $file_name ) ) . '</td>';
		$ppom_html .= '</tr>';

		// Checking if cropped file existing
		$cropped_file_name = self::file_get_name( $file_name, $product_id );
		if ( $cropped && file_exists( self::get_dir_path( 'cropped' ) . $cropped_file_name ) ) {

			$cropped_url = self::get_dir_url() . 'cropped/' . $cropped_file_name;
			$ppom_html  .= '<tr>';
	 ...
	 */
	public static function create_chunk_file( $file_path_to_read, $ppom_chunk_file_path, $mode ) {
		...
		if ( $chunk_file ) {
			...
			if ( $temp_file ) {
				...
				return UploadErrors::OPEN_INPUT;
			}
			fclose( $temp_file );
			fclose( $chunk_file );
		} else {
			return UploadErrors::OPEN_OUTPUT;
		}
	 ...
	 * @return void
	 *
	 * @see self::files_setup_get_directory()
	...
	public static function upload_file() {
		...
		if ( $error ) {
			die( UploadErrors::get_message_response( $error ) );
		}
			...
			if ( self::is_file_image( $file_path ) ) {
				...
				if ( file_exists( $thumb_dir_path ) ) {
					...
					$response        = array(
						...
						'file_h'    => $fh,
						'nocache'   => time(),
						'file_url'  => self::get_dir_url() . $file_name,
						'html'      => self::uploaded_file_preview( $file_name, $file_meta ),
					);
				} else {
					$response = array(
	...
	public static function create_image_thumb( $file_path, $image_name, $thumb_size ) {
	...
	public static function uploaded_file_preview( $file_name, $settings ) {
		...
		$short_name = self::files_trim_name( $file_name );
	 ...
	 * @return string
	 */
	public static function files_trim_name( $file_name ) {
		return $file_name;
	}


	// Save cropped image fro dataUrl to image
	public static function save_data_url_to_image( $data, $file_name ) {

		$data = base64_decode( preg_replace( '#^data:image/\w+;base64,#i', '', $data ) );

		$dest_cropped = self::get_dir_path( 'cropped' ) . $file_name;
		file_put_contents( $dest_cropped, $data );
	}

	// Building options array for croppie
	public static function get_croppie_options( $settings ) {
		...
		$viewport_settings = self::get_viewport_settings( $settings );
	...
	public static function get_viewport_settings( $settings ) {
	...
	*/
	public static function files_removed_unused_images() {
		...
		if ( is_dir( $dir ) && apply_filters( 'ppom_remove_unused_images', true ) ) {
			...
			while ( $file = readdir( $dir_handle ) ) {
				...
				if ( is_file( $file_path ) ) {
					...
					$day_count = self::files_uploaded_days_count( $file_created_date, $today );
	...
	public static function files_uploaded_days_count( $date1, $date2 ) {
	...
	public static function file_get_name( $file_name, $product_id, $cart_item = null ) {
