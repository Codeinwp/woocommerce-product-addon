 ...
 * @subpackage Pricing
 *
 * @see self::price_controller()
 * @see self::get_field_prices()
...
final class Engine {
	 ...
	 * @see self::price_cart_fee()
	 */
	public static function get_field_prices( $ppom_fields_post, $product_id, &$product_quantity, $variation_id, $item = null ) {
		...
		foreach ( $ppom_fields_post as $data_name => $value ) {
			...
			switch ( $field_type ) {
				...
				case 'email':
					...
					if ( $field_price !== '' ) {
						$field_prices[] = self::generate_field_price( $field_price, $field_meta, $charge, $option, $option_quantity );
					}
					break;
					...
					foreach ( $options as $option ) {
						...
						if ( $option_raw == stripcslashes( $value ) ) {
							...
							if ( $option_price !== '' ) {
								...
								if ( $option_percent !== '' ) {
									$option_price = self::get_amount_after_percentage( $product_price, $option_percent );
								}

								$field_prices[] = self::generate_field_price( $option_price, $field_meta, $charge, $option, $option_quantity );
							}

							// weight
							if ( ! empty( $option['option_weight'] ) ) {
								$field_price    = 0;
								$field_prices[] = self::generate_field_price( $field_price, $field_meta, 'weight', $option );
							}
						}
					}
					break;
								...
								if ( $option_raw == stripcslashes( $val ) ) {
									...
									if ( $option_price !== '' ) {
										if ( strpos( $option_price, '%' ) !== false ) {
											$option_price = self::get_amount_after_percentage( $product_price, $option_price );
										}
										$field_prices[] = self::generate_field_price( $option_price, $field_meta, $charge, $option, $option_quantity );
									}
				...
				case 'fancycropper':
					foreach ( $value as $popupID => $image_data ) {
						...
						if ( $cropped_price > 0 ) {
							...
							$field_prices[] = self::generate_field_price( $cropped_price, $field_meta, $charge, $image_data, $option_quantity );
						}
					}
				...
				case 'image':
					...
					// $field_prices[] = self::generate_field_price($option_price, $field_meta, $charge, $option);
					break;
				...
				case 'file':
					...
					if ( $field_price > 0 ) {
						...
						$field_prices[] = self::generate_field_price( $field_price, $field_meta, $charge, $option, 1 );
					}
					break;
				...
				case 'quantities':
				case 'qtypack':
					// ppom_pa(self::is_field_has_price($field_meta));

					if ( self::is_field_has_price( $field_meta ) ) {
						foreach ( $options as $option ) {
							$quantities_total = 0;
							...
							foreach ( $value as $val => $quantity ) {
								...
								if ( $option['raw'] == $val && $quantity > 0 ) {
									...
									$field_prices[] = self::generate_field_price( $option_price, $field_meta, $charge, $option, $quantity );

									// weight
									if ( ! empty( $option['option_weight'] ) ) {
										$field_price    = 0;
										$field_prices[] = self::generate_field_price( $field_price, $field_meta, 'weight', $option, $quantity );
									}
								}
							}
						}
						...
						if ( $pricematrix_field ) {
							...
							if ( $matrix_found ) {
								...
								$field_prices[] = self::generate_field_price( $option_price, $field_meta, $charge, $option, $product_quantity );
							}
						}               
					}
						...
						foreach ( $vm_options as $vm_key => $vm_price ) {
							...
							foreach ( $value as $val => $quantity ) {
								...
								if ( $val === $vm_key && $quantity > 0 ) {
									...
									$field_prices[] = self::generate_field_price( $option_price, $field_meta, $charge, $options, $quantity );
								}
							}
						}
				...
				case 'bulkquantity':
					...
					$bq_found     = self::price_bulkquantity_chunk( $product, $options, $product_quantity );
					$option_price = isset( $bq_found[ $bq_value ] ) ? floatval( $bq_found[ $bq_value ] ) : null;
					if ( $option_price ) {
						$single_quantity = 1;
						$field_prices[]  = self::generate_field_price( $option_price, $field_meta, $charge, $bq_found, $single_quantity );
					}
					// ppom_pa($bq_found);
					break;

				case 'fixedprice':
					$fp_found   = self::price_fixedprice_chunk( $product, $options, $product_quantity );
					$unit_price = PPOM_FPP()->get_unit_price( $fp_found, $field_meta );
					if ( $unit_price ) {
						$field_prices[] = self::generate_field_price( $unit_price, $field_meta, $charge, $fp_found, 1 );
					}
					break;
				...
				case 'measure':
					...
					$unit_price          = 0;
					$measure_price_field = self::generate_field_price( $unit_price, $field_meta, $charge, $options, $product_quantity );
				...
				case 'eventcalendar':
					// if( self::is_field_has_price($field_meta) ) { // Waite & Watch Party
					if ( 1 ) {
							...
							foreach ( $value as $date => $ticket_meta ) {
								...
								foreach ( $ticket_meta as $label => $qty ) {
									...
									if ( $ticket_label == $label ) {
										...
										$field_prices[] = self::generate_field_price( $ticket_price, $field_meta, $charge, $options, $qty );
									}
								}
							}
	...
	public static function generate_field_price( $field_price, $field_meta, $apply, $option = array(), $qty = 0 ) {
	...
	public static function price_get_addon_total( $price_array ) {
	...
	public static function price_get_cart_fee_total( $price_array ) {
	...
	public static function price_get_total_bulkquantities( $price_array, $product, $ppom_fields_post ) {
	...
	public static function price_get_total_fixedprice( $price_array ) {
	...
	// Get total fixedprice
	public static function price_get_total_measure( $price_array ) {
		$total_measure = 0;
	 ...
	 *
	 * @see self::get_field_prices()
	 * @see self::parse_price_matrix()
	 * @see self::price_controller()
	 */
	public static function price_get_product_base(
		$product_price,
		$product,
		$ppom_fields_post,
		$product_quantity,
		...
		$quantities_found     = self::price_get_total_quantities( $ppom_fields_post, $product_id );
		$bulkquantities_found = self::price_get_total_bulkquantities( $ppom_field_prices, $product, $ppom_fields_post );
		$fixedprice_found     = self::price_get_total_fixedprice( $ppom_field_prices );
		$measure_found        = self::price_get_total_measure( $ppom_field_prices );
	...
	public static function price_matrix_chunk( $product, $pricematrix_fields, $product_quantity ) {
	...
	// If Bulkquantity add-on is used, get it's chunk
	public static function price_fixedprice_chunk( $product, $fixedprice_options, $product_quantity ) {
	 ...
	 *
	 * @see self::get_field_prices()
	 * @see self::price_has_discount_matrix()
	 * @see self::price_controller()
	 */
	public static function price_cart_fee( $cart ) {
		$fee_no       = 1;
		$cart_counter = 1;
		foreach ( $cart->get_cart() as $item ) {
			...
			$ppom_field_prices = self::get_field_prices( $ppom_fields_post, $product_id, $quantity, $variation_id, $item );
			// ppom_pa($ppom_field_prices);
			...
			if ( $matrix_found = self::price_has_discount_matrix( $product, $quantity ) ) {
				...
				if ( ! empty( $matrix_found['percent'] ) ) {
					$matrix_discount = self::get_amount_after_percentage( $price_tobe_discount, $matrix_found['percent'] );
				} else {
	...
	public static function is_field_has_price( $meta ) {
	...
	public static function price_has_discount_matrix( $product, $quantity ) {
		...
		$matrix_found = self::price_matrix_chunk( $product, $pricematrix_field, $quantity );
		// ppom_pa($matrix_found);
	...
	public static function get_amount_after_percentage( $base_amount, $percent ) {
	...
	public static function price_check_price_matrix( $cart_items, $values ) {
	...
	public static function option_price_handle_vat( $option_price, $product ) {
	...
	public static function wwp_product_cart_price( $product_price, $cart_content ) {
