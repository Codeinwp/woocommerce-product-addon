 ...
 * @subpackage Helpers
 *
 * @see self::make_meta_data()
 * @see self::generate_cart_meta()
 * @see self::get_field_meta_by_dataname()
 */
...
final class Helpers {
	...
	public static function direct_access_not_allowed() {
	}


	public static function pa( $arr ) {
	...
	// Get field column
	public static function get_field_colum( $meta ) {
	...
	public static function translation_options( $option ) {
	...
	public static function get_product_price( $product, $variation_id = null, $context = '' ) {
	 ...
	 */
	public static function recursive_sanitization( $field_value ) {
		...
		if ( is_array( $field_value ) ) {
			foreach ( $field_value as $key => $val ) {
				$field_value[ $key ] = self::recursive_sanitization( $val );
			}
		}
	 ...
	 * delegating display formatting to `self::generate_cart_meta()`.
	 *
	 ...
	 */
	public static function make_meta_data( $cart_item, $context = 'cart' ) {
		...
		if ( is_array( $ppom->fields ) ) {
			foreach ( $ppom->fields as $field ) {
				...
				if ( isset( $cart_item['ppom']['fields'][ $data_name ] ) ) {
					$cart_item['ppom']['fields'][ $data_name ] = self::recursive_sanitization( $cart_item['ppom']['fields'][ $data_name ] );
				}
			}
		}
		...
		// self::pa($cart_item);
		$ppom_meta = self::generate_cart_meta( $ppom_cart_fields, $product_id, $ppom_meta_ids, $context );
	 ...
	 * @see self::get_field_meta_by_dataname()
	 */
	public static function generate_cart_meta( $ppom_cart_items, $product_id, $ppom_meta_ids = null, $context = 'cart', $variation_id = null ) {
		$ppom_meta = array();
		...
		foreach ( $ppom_cart_fields as $key => $value ) {
			...
			$conditionally_hidden = isset( $ppom_cart_items['conditionally_hidden'] ) ? $ppom_cart_items['conditionally_hidden'] : '';
			if ( self::is_field_hidden_by_condition( $key, $conditionally_hidden ) ) {
				continue;
			}

			// $cart_item['data'] ->post_type == 'product' ? $cart_item['data']->get_id() : $cart_item['data']->get_parent_id();
			$field_meta = self::get_field_meta_by_dataname( $product_id, $key, $ppom_meta_ids );
			$data_name  = $key;
			// self::pa($value);
			...
			switch ( $field_type ) {
				...
				case 'fancycropper':
					...
					foreach ( $value as $popupID => $image_data ) {
						...
						$ppom_html .= '<tr>';
						$ppom_html .= '<td>';
						$ppom_html .= '<a href="' . esc_url( $imageURL ) . '"><img class="img-thumbnail" style="width:' . esc_attr( self::get_thumbs_size() ) . '" src="' . esc_url( $imageURL ) . '" title="' . esc_attr( $fileName ) . '"></a>';
						$ppom_html .= '</td>';
						...
						foreach ( $fomatted_data['cropped'] as $cropped_id => $cropped_meta ) {
							...
							$ppom_html .= '<img class="img-thumbnail" style="width:' . esc_attr( self::get_thumbs_size() ) . '" src="' . esc_url( $cropped_url ) . '">';
							$ppom_html .= '</a>';
							$ppom_html .= '</td>';
				...
				case 'cropper':
					...
					$crop_options = self::convert_options_to_key_val( $field_meta['options'], $field_meta, $product );
					$crop_size    = '';
				...
				case 'image':
					if ( $value ) {
						...
						$display = self::generate_html_for_images( $value );
				...
				case 'palettes':
					...
					$options_filter = self::convert_options_to_key_val( $field_meta['options'], $field_meta, $product );
			...
			$option_price = self::get_field_option_price( $field_meta, $value );
			if ( $option_price != 0 ) {
	 ...
	 * @since 8.2
	 **/
	public static function meta_priced_options( $the_meta ) {
	 ...
	 **/
	public static function if_browser_is_ie() {
	 ...
	 **/
	public static function has_posted_field_value( $posted_fields, $field ) {
	...
	public static function is_aviary_installed() {
	 ...
	 * @see self::generate_cart_meta()
	 */
	public static function get_field_meta_by_dataname( $product_id, $original_data_name, $ppom_id = null ) {
		...
		if ( self::pro_is_installed() && self::check_pro_compatibility( 'cond_field_repeat' ) && ! self::check_pro_compatibility( 'pgfbdfm_wp_filter_param_fix' ) ) {
	...
	public static function has_field_by_type( $product_id, $field_type ) {
	...
	public static function load_input_templates( $template_path, $vars = array( '' ) ) {
	...
	public static function load_file( $file_path, $variables = array( '' ) ) {
	...
	public static function load_bootstrap_css() {
		...
		$return = true;
		if ( self::get_option( 'ppom_disable_bootstrap' ) == 'yes' ) {
			$return = false;
		}
	...
	public static function load_fontawesome() {

		$return = true;
		if ( self::get_option( 'ppom_disable_fontawesome' ) == 'yes' ) {
			$return = false;
		}
	...
	public static function convert_options_to_key_val( $options, $meta, $product ) {
		...
		foreach ( $options as $option ) {
			...
			$the_option = isset( $option['option'] ) ? stripslashes( wp_specialchars_decode( $option['option'] ) ) : '';

			if ( apply_filters( 'ppom_hide_if_out_of_stock', true, $option ) ) {
				$has_stock = self::option_has_stock( $option );
				if ( ! $has_stock ) {
					continue;
				}
			}
			...
			if ( $the_option != '' ) {
				...
				$option                   = self::translation_options( $option );
				...
				$option_label = self::generate_option_label( $option, $option_price, $meta );
				...
				if(strpos($option_price,'%') !== false){
					...
					if(isset($meta['onetime']) && $meta['onetime'] == 'on' && isset($meta['onetime_taxable']) && $meta['onetime_taxable'] == 'on') {
						...
						$option_price = self::get_price_including_tax($option_price, $product);
					}
					$option_label   = self::generate_option_label($option, $option_price, $meta);
				...
				// self::pa($option);
				$option_id = self::get_option_id( $option, $meta );
	...
	public static function generate_option_label( $option, $price, $meta ) {
		...
		if ( ! empty( $price ) ) {
			...
			$price = strip_tags( self::price( $price ) );
	...
	public static function get_option_id( $option, $field_meta = null ) {
	...
	public static function get_price_including_tax( $price, $product ) {
	...
	public static function is_field_hidden_by_condition( $field_name, $conditionally_hidden = null ) {

		if ( ! isset( $_POST['ppom']['conditionally_hidden'] ) && $conditionally_hidden == null ) {
			return false;
		}
	...
	public static function get_cart_item_max_quantity( $cart_item ) {
	...
	public static function extract_matrix_by_quantity( $quantities_field, $product, $quantity ) {

		$matrix = '';
		if ( ! isset( $quantities_field['options'] ) ) {
			return $matrix;
		}

		$options = $quantities_field['options'];
		$ranges  = self::convert_options_to_key_val( $options, $quantities_field, $product );
	 ...
	 * @return string
	 */
	public static function get_thumbs_size( $size = 150 ) {
	...
	public static function generate_html_for_files( $file_names, $input_type, $item ) {
	...
	public static function generate_html_for_images( $images ) {
		global $post;
		...
		if ( $ppom_has_cart_block ) {
			return self::get_image_name( $images );
		}
	...
	public static function get_field_option_price_by_id( $option, $product, $ppom_meta_ids ) {
		...
		$product_id = null;
		$field_meta = self::get_field_meta_by_dataname( $product_id, $data_name, $ppom_meta_ids );
		...
		if ( ! isset( $field_meta[ $option_key ][0]['id'] ) ) {
			$field_meta[ $option_key ] = self::convert_options_to_key_val( $field_meta[ $option_key ], $field_meta, $product );
		}

		$field_type = isset( $field_meta['type'] ) ? $field_meta['type'] : '';

		if ( $field_type == 'bulkquantity' || $field_type == 'cropper' ) {
			return 0;
		}
	...
	public static function get_field_option_weight_by_id( $option, $ppom_meta_ids ) {
	...
	public static function field_has_stock( $meta, $value ) {
	...
	public static function option_has_stock( $option ) {
	 ...
	 * @return bool
	 */
	public static function pro_is_valid_license() {
		return self::pro_is_installed() && apply_filters( 'product_ppom_license_status', '' ) === 'valid';
	}

	// Check if PPOM API is enable
	public static function is_api_enable() {

		$api_enable = self::get_option( 'ppom_api_enable' );
		$api_key    = self::get_option( 'ppom_rest_secret_key' );
	...
	public static function is_field_visible( $field ) {
		...
		switch ( $visibility ) {
			...
			case 'roles':
				...
				// var_dump(array_intersect($allowed_roles, $user_roles));
	...
	public static function get_price_matrix_chunk( $product, $option_prices, $ppom_item_order_qty ) {

		$matrix_found = '';

		$pricematrix_field = self::has_field_by_type( self::get_product_id( $product ), 'pricematrix' );
		// self::pa($pricematrix_field);
		...
		if ( count( $pricematrix_field ) > 0 ) {
			...
			$matrix_found = self::extract_matrix_by_quantity( $pricematrix_field, $product, $ppom_item_order_qty );
		}
	...
	public static function is_price_attached_with_fields( $fields_posted ) {
				...
				case 'checkbox':
					if ( is_array( $value ) ) {
						foreach ( $value as $cb_value ) {
							$option_price += self::get_field_option_price( $field_meta, $cb_value );
						}
					}
					break;
	...
	public static function get_price_mode() {
		...
		$price_mode = 'new';
		if ( self::get_option( 'ppom_legacy_price' ) == 'yes' ) {
			$price_mode = 'legacy';
		}
	...
	public static function is_client_validation_enabled() {

		$validation = false;
		if ( self::get_option( 'ppom_enable_client_validation' ) == 'yes' ) {
			$validation = true;
		}
	...
	public static function get_conditions_mode() {

		$mode = 'new';
		if ( self::get_option( 'ppom_new_conditions' ) == 'yes' ) {
			$mode = 'legacy';
		}
	...
	public static function get_price_table_calculation() {

		$js_script = 'ppom-price.js';
		if ( self::get_option( 'ppom_price_table_v2' ) == 'yes' ) {
			$js_script = 'ppom-price-v2.js';
		}
	 ...
	 * @return string[]
	 */
	public static function get_price_table_js_dependencies() {
	...
	public static function get_price_table_location() {

		$location = self::get_option( 'ppom_price_table_location', 'after' );
	...
	public static function is_cart_quantity_updatable( $product_id ) {

		$qty_updatable = true;

		$ppom = new PPOM_Meta( $product_id );
		if ( ! $ppom->fields ) {
			return $qty_updatable;
		}
	 ...
	 * only use 1
	 * */
	public static function reset_cart_quantity_to_one( $product_id ) {
	...
	public static function attach_fields_to_product( $ppom_meta_id, $product_id ) {
	...
	public static function get_all_editable_roles() {
	...
	public static function security_role() {

		$action        = false;
		$ppom_security = array();
		$ppom_security = self::get_option( 'ppom_permission_mfields', array() );
	...
	public static function get_conditional_data_attributes( $meta ) {
		...
		if ( isset( $conditions['rules'] ) && $logic === 'on' ) {
			...
			$index       = 0;
			$pro_enabled = self::pro_is_installed() && 'valid' === apply_filters( 'product_ppom_license_status', '' );
	...
	public static function is_legacy_mode() {
		...
		$is_legacy     = false;
		$enable_legacy = self::get_option( 'ppom_enable_legacy_inputs_rendering' );
	...
	public static function settings_migrated() {
	 ...
	 * @return bool
	 */
	public static function check_pro_compatibility( $feature_slug ) {
	 ...
	 * @return bool
	 */
	public static function is_legacy_user() {
		if ( self::pro_is_installed() && ( function_exists( '\PPOM_Pro\get_license_status' ) && 'valid' === \PPOM_Pro\get_license_status( false ) ) ) {
			return false;
		}
	 ...
	 */
	public static function posted_field_max_min_value_validation( $posted_fields, $field ) {
	 ...
	 * @return string
	 */
	public static function get_image_name( $images ) {
		$updated_value = '';
