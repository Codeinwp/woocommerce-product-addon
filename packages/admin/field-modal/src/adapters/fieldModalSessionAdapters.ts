...
import {
  ...
  commitFieldsToClassicForm,
  getClassicDraftFields,
...
export interface FieldModalTransportAdapter {
  ...
  fetchSchema: (type: string) => Promise<Record<string, unknown> | null>;
  saveGroup: (args: {
    productmetaId: number | undefined;
...
export interface FieldModalAdminAdapter {
  ...
  commitFieldsToClassicForm: (
    fields: Array<Omit<FieldRow, "clientId">>,
  ) => void;
}
...
export const defaultFieldModalSessionAdapters: FieldModalSessionAdapters = {
  transport: {
    fetchContext: async (productmetaId) => {
      const ctx = await fetchFieldModalContext(productmetaId);
      const draftFields = getClassicDraftFields();
      if (draftFields) {
        return {
          ...ctx,
          fields: draftFields as FieldRow[],
        };
      }
      return ctx;
    },
